...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pv-filter { padding: 5px 12px; border-radius: 999px; font-size: 11px; font-weight: 700; border: 1.5px solid #e2e8f0; background: #fff; color: #64748b; cursor: pointer; font-family: inherit; white-space: nowrap; transition: all .15s; }
...
export default function PublicView() {
  ...
  async function openDetail(proj) {
    ...
    const res = await fetch(`${API}/public/company/${proj.companyId}`)
      .then(r => r.json()).catch(() => null);
    setDetail({ ...proj, companyDetail: res });
    setDetailLoading(false);
  }

  // FIX: use typeStr not t (t conflicts with translation object)
  const types = ["all", ...new Set(projects.map(proj => proj.type).filter(Boolean))];

  const filtered = projects.filter(proj => {
    // FIX: use qStr consistently
  ...
  return (
    <div className="pv">
      ...
      <div className="pv-inner">
        ...
        {/* Filter chips — FIX: use typeStr not t */}
        <div className="pv-filters">
          {types.map(typeStr => (
            <button key={typeStr}
              className={`pv-filter ${filter === typeStr ? "active" : ""}`}
              onClick={() => setFilter(typeStr)}>
              {typeStr === "all" ? "🌍 Semua" : `${TYPE_ICON[typeStr] || "🌿"} ${typeStr}`}
            </button>
          ))}
        </div>
          ...
          <div className="pv-hero-stats">
            {[
              ...
              <div key={i} className="pv-hero-stat">
                <div className="pv-hero-stat-val">{stat.val}</div>
        ...
        <div className="pv-main">
          {loading ? (
            ...
            <div key={proj.id || idx} className="pv-card fade-up" onClick={() => openDetail(proj)}
              ...
              <div className="pv-card-header">
                ...
                <div className="pv-card-info">
                  ...
                  <div className="pv-card-meta">{proj.flag} {proj.country} · {proj.type}</div>
                </div>
              ...
              <div className="pv-metrics">
                {[
                  ...
                  { label: "Total",   val: proj.totalAbsorption ? `${proj.totalAbsorption} t/bln` : "—" },
                  { label: "Rating",  val: `⭐ ${proj.rating}` },
                ].map((metric, i) => (
                  <div key={i} className="pv-metric">
                    <div className="pv-metric-val">{metric.val}</div>
                    <div className="pv-metric-label">{metric.label}</div>
                  </div>
                ))}
              </div>
              <div className="pv-card-badges">
                ...
                {proj.isoVerified && <span className="pv-badge pv-badge-blue">🔒 Terverifikasi</span>}
                {proj.companyESG && (
                  <span className="pv-badge" style={{
                    background: proj.companyESG >= 70 ? "#dcfce7" : "#fef3c7",
                    color:      proj.companyESG >= 70 ? "#166534"  : "#92400e",
                  }}>ESG {proj.companyESG}</span>
                )}
                <span style={{ marginLeft:"auto", fontSize:11, color:"#94a3b8" }}>Lihat detail →</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="pv-overlay" onClick={() => setDetail(null)}>
          <div className="pv-sheet" onClick={e => e.stopPropagation()}>
            ...
            <div className="pv-sheet-body">
              {detailLoading ? (
                <div style={{ textAlign:"center", padding:"24px 0", color:"#94a3b8", fontSize:13 }}>Memuat detail...</div>
              ) : (
                <>
                  <div>
                    ...
                    <div className="pv-info-grid">
                      {[
                        ...
                        { l:"Total Area",    v: detail.companyDetail
                            ? `${detail.companyDetail.parcels?.reduce((s, p) => s + (p.area||0), 0)?.toLocaleString()} ha`
                            : "—" },
                      ].map((item, i) => (
                  ...
                  {/* FIX: use pItem.name consistently */}
                        ...
                        {detail.companyDetail.parcels.map(pItem => (
                          <div key={pItem.id} className="pv-parcel">
                            ...
                            <div style={{ flex:1, minWidth:0 }}>
                              <div className="pv-parcel-name" style={{ overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                                {pItem.name}
                              </div>
                              <div className="pv-parcel-sub">{pItem.area} ha · {pItem.type} · {pItem.status}</div>
                            </div>
                            <div className="pv-parcel-abs"
                              style={{ color: (pItem.absorptionMonthly || 0) >= 0 ? "#166534" : "#dc2626" }}>
                              {(pItem.absorptionMonthly || 0) >= 0 ? "▲" : "▼"}{Math.abs(pItem.absorptionMonthly || 0)} t/bln
                            </div>
                          </div>
                        ))}
                      </div>
                  ...
                  {detail.companyDetail && (
                    <div style={{ background:"#f8fafc", borderRadius:12, padding:"12px 14px", display:"flex", flexDirection:"column", gap:8 }}>
                      ...
                      {[
                        ...
                        { l:"Net / bulan",           v:`${(detail.companyDetail.totalAbsorption - detail.companyDetail.totalEmission).toFixed(2)} tCO₂`, c:"#1e293b" },
